...
export function VideoCallScreen({ session, character, onEnd, onConnect, onMinimize, initiator = "user", offlineMode }: VideoCallScreenProps) {
    ...
    useEffect(() => {
        ...
        const initRole = initiator === "character" ? "assistant" : "user";
               if (!lastMsg || !(lastMsg.content.includes("发起了视频通话") || lastMsg.content.includes("面对面相处"))) {
    ...
    const runConversationTurn = useCallback(async (userText?: string) => {
        ...
        try {
            ...
            const aiResponseText = flattenCompletionResult(await generateChatCompletion(session, messagesRef.current, {
               appTags: offlineMode ? ["chat", "offline"] : ["chat", "video"],
        ...
        <div className="absolute inset-0 z-[100] flex flex-col bg-black text-white overflow-hidden call-keyboard-shift" style={keyboardOffsetStyle}>
            ...
            <div className="relative z-10 gcall-topbar gcall-topbar-video">
                <div className="gcall-topbar-title videocall-name flex items-center justify-between w-full px-4">
                    <button onClick={() => onMinimize?.(callDuration)} className="ui-bare-btn text-white/70 hover:text-white transition-colors" title="合上">
