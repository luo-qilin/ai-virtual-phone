...
async function fetchJson<T>(input: RequestInfo | URL, init?: RequestInit, timeoutMs = 15000): Promise<T> {
  ...
  try {
    ...
    if (err instanceof DOMException && err.name === "AbortError") throw new Error("请求超时，请检查网络连接。");
    throw err;
  } finally {
...
export async function updateCustomAppMarketItem(input: { id: string; appId?: string; app?: InstalledCustomApp; name?: string; packageUrl?: string; packagePath?: string; packageKind?: CustomAppPackageKind; packageSize?: number; version?: string; description?: string; iconDataUrl?: string; changelog?: string }): Promise<CustomAppMarketItem> { const version = input.version?.trim() || input.app?.version; const rawIcon = input.iconDataUrl ?? input.app?.iconDataUrl;const data = await fetchJson<MarketListResponse>("/api/app-market/apps", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: input.id, appId: input.appId, name: input.name ?? input.app?.name, version, changelog: input.changelog, description: input.description ?? input.app?.description, iconDataUrl: rawIcon === undefined ? undefined : await compressIconForMarket(rawIcon), permissions: input.app?.permissions, manifest: input.app ? { ...input.app.manifest, version: version || input.app.version, primaryTags: getCustomAppPrimaryTags(input.app) } : undefined, packageUrl: input.packageUrl, packagePath: input.packagePath, packageKind: input.packageKind, packageSize: input.packageSize }) }, 60000); if (!data.app) throw new Error(data.error || "应用更新失败"); return data.app; }
